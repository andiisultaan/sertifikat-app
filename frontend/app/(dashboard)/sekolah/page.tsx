...
import { useCreateSekolah, useDeleteSekolah, useSekolahList, useUpdateSekolah } from "@/lib/hooks/useSekolah";
    ...
    createSekolah(
      { nama: newSekolahName },
      {
        onSuccess: (res) => {
          setCreateOpen(false);
          setNewSekolahName("");
          setCredential({ email: res.admin.email, password: res.admin.password });
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Cukup masukkan nama sekolah. Akun admin sekolah akan dibuat otomatis.</DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={!!credential} onOpenChange={(v) => !v && setCredential(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Simpan kredensial admin berikut. Password tidak bisa ditampilkan lagi.</DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={editOpen} onOpenChange={(v) => !v && setEditOpen(false)}>
        <DialogContent className="max-w-xl">
          ...
          {selectedSekolah?.admin ? (
            <p className="text-xs text-muted-foreground">Admin sekolah: {selectedSekolah.admin.name} ({selectedSekolah.admin.email})</p>
          ) : null}
        </DialogContent>
      </Dialog>
          ...
          {isLoading ? (
            ...
            <tbody className="divide-y">
              {sekolahList?.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-4 py-8 text-center text-gray-400">Belum ada data sekolah.</td>
                </tr>
              ) : null}
              {sekolahList?.map((s) => (
